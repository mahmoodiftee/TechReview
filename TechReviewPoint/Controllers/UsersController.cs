...
namespace TechReviewPoint.Controllers
{
    public class UsersController : Controller
    {
        ...
        {
            if (user.user_img_file != null && user.user_img_file.ContentLength > 0) // Check if a file is uploaded
            {
                string fileName, extension;
                NewMethod(user, out fileName, out extension);
        ...
        private static void NewMethod(User user, out string fileName, out string extension)
        {
            fileName = "";
            extension = "";

            if (user.user_img_file != null)
            {
                fileName = Path.GetFileNameWithoutExtension(user.user_img_file.FileName);
                extension = Path.GetExtension(user.user_img_file.FileName);
            }
        }
